...
React Compiler helps by automatically memoizing the component's overall output, but does not memoize individual callbacks in loops since each captures a different `item`. Child components still benefit from `React.memo`.

</Note>

### After React Compiler {/*after-react-compiler*/}

With React Compiler, you can simplify your code by removing much of the manual memoization:

```js
import { memo } from 'react';

const Item = memo(function Item({ onClick }) {
  // Item implementation
});
...
React Compiler automatically memoizes the component's overall output and expensive calculations like `expensiveProcessing(data)`.
